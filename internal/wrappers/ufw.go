...
func (u *UFWWrapper) Status(ctx context.Context) string {
cmd := exec.CommandContext(ctx, "ufw", "status")
out, err := cmd.CombinedOutput()
outStr := string(out)
if err != nil {
if strings.Contains(err.Error(), "executable file not found") || strings.Contains(outStr, "command not found") {
return "not_installed"
}
}
if strings.Contains(outStr, "inactive") {
return "inactive"
}
return "active"
}

func (u *UFWWrapper) Toggle(ctx context.Context, enable bool) error {
action := "disable"
if enable {
action = "enable"
}
cmd := exec.CommandContext(ctx, "ufw", "--force", action)
return cmd.Run()
}
